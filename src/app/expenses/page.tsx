          ...
          <div className="px-4 py-3 bg-[#f9f9f9] border-t" style={{ borderColor: activeMeta.color + "25" }}>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative shrink-0 w-12 h-6 rounded-full transition-colors duration-200 ${applyGst ? "bg-black" : "bg-[#ddd]"}`}
                role="switch" aria-checked={applyGst}
              >
                <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${applyGst ? "translate-x-6" : "translate-x-0"}`} />
              </button>
            </div>
            {applyGst && (
